<template>
  <div>
    ...
    <group title="咨询信息" label-width="5em" label-align="left">
      <cell title="问题标题" v-model="consult.title"></cell>
      <x-textarea title="详细信息" v-model="consult.content" readonly :show-counter="false"
                  :rows="4"></x-textarea>
    ...
    <group title="处理进度" label-width="5em" label-align="left">
      ...
      <x-textarea v-if="consult.status !== 0" title="回复信息" v-model="consult.reply" readonly
                  :show-counter="false"
                  :rows="4"></x-textarea>
    ...
    <box gap="10px 10px">
      <flexbox>
        <flexbox-item v-if="consult.status !== 2">
...
<script>
  ...
  export default {
    ...
    computed: {
      formateTime () {
        return this.consult.createTime.replace('T', ' ')
      },
      updateTime () {
        return this.consult.updateTime.replace('T', ' ')
      },
      statusDesc () {
        switch (this.consult.status) {
          case 0:
            return '未处理'
          case 1:
            return '处理中'
    ...
    methods: {
      ...
      remove () {
        this.$http.post('/consultMessage/deleteConsultMessage', this.consult).then(response => {
          if (response.code === 0) {
      ...
      edit () {
        this.$router.push({name: 'EditConsultMessage', params: {id: this.consult.id}})
      }
    },
    data () {
      return {
